...
export const timerHandler = {
  ...
  updateEstimatedTime: (search, searchConfig) => {
    ...
    let complete = new Date(
      now.getTime() +
        searchConfig.interval * (searchConfig.limit - search.index),
    );
    if (search.index === searchConfig.limit) next = now;
  ...
  run: (elements, searchConfig, searchEngine) => {
    ...
    if (searchConfig.interval === 9999) {
      ...
      if (
        timerHandler.searchHandler &&
        timerHandler.searchHandler.getNextTerm
      ) {
        nextTerm = timerHandler.searchHandler.getNextTerm(
          timerHandler.currentSearchIndex,
        );
            ...
            <div class="next-terms flex flex-wrap justify-center gap-1 pb-1">
              <div class="badge pr-0">
                ...
                <div class="badge badge-success mx-0 translate-x-[2px] px-2">
                  ...
                  ${
                    nextTermIsTruncated
                      ? `<div class="tooltip tooltip-left tooltip-neutral" data-tip="${nextTerm}">
      ...
      const nextSearchTimer =
        elements.div.timer.querySelector(".next-search-timer");
      const completeTimer = elements.div.timer.querySelector(".complete-timer");
      const nextTermElement =
        elements.div.timer.querySelector(".next-term-text");
      ...
      if (nextTermElement) {
        ...
        const tooltipContainer =
          elements.div.timer.querySelector(".next-terms .badge");
